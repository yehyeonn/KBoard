...
@Configuration  // 자체도 빈, 안에도 빈 생성 가능
@EnableWebSecurity
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        // URL 과 접근권한 세팅(들)
                        // ↓ /board/detail/** URL로 들어오는 요청은 '인증'만 필요. = 로그인만 되면 권한 따지지 않고 허용한다.
                        ...
                        // 403 에러는 보안과 관련된 에러, 이제 인증이나 권한 가져야 하는 링크는 창 띄우는 행동 필요
                )
                /********************************************
                 * ② 폼 로그인 설정
...
//                        .usernameParameter("aaa")   //html에서 name 값이 aaa 인 것을 가져온다. 기본 name = "username"
...
// ※ 아래 설정 없이도 기본적올 /logout 으로 로그아웃 된다
                ...
                .logout(httpSecurityLogoutConfigurer -> httpSecurityLogoutConfigurer
                        ...
                        .logoutSuccessUrl("/home")  // 로그아웃 성공 후 redirect url       그러나 아래 거 달면 로그아웃과 동시에 세션 날려서 동작 하지 않음.
...
//                        .deleteCookies("JSESSIONID") // 특정 쿠키 제고
