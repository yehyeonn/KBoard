...
//    private PasswordEncoder passwordEncoder;    // 회원 가입시 PW 인코딩
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        OAuth2User oAuth2User = super.loadUser(userRequest);    // 사용자 프로필 정보(구글 로그인 시 넘어온 것), OAuth2User 객체 리턴
        ...
        String username = provider + "_" + providerId;  // ex) "google-xxxx"
        ...
        if(user == null){   // 미가입자인 경우, 회원가입 진행
            ...
            if(cnt > 0) {
                ...
                user = userService.findByUsername(username);    // 다시 읽어온다. regDate 정보 등을 위해
            }
        ...
        PrincipalDetails principalDetails = new PrincipalDetails(user, oAuth2User.getAttributes()); // 아까 Principaldetails 에서 오버라이딩함! 기본 회원, OAuth2 회원
