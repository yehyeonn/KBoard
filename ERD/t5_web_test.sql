...
SELECT a.id "id", a.name "name"
    ...
    WHERE a.id = u.authority_id AND u.user_id = 1;  -- 1번 유저의 권한 가져온다
...
SELECT * FROM t5_post ORDER BY id desc;
SELECT * FROM t5_post ORDER BY id desc LIMIT 5; -- 5개
SELECT * FROM t5_post ORDER BY id desc LIMIT 5, 5;  -- 5번째부터 5개
